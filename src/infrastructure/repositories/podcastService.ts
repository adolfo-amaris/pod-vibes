...
import { PodcastAPIResponse, PodcastDetailsAPIResponse, PodcastDetailsRawAPIResponse, PodcastFeedResponse } from '../../domain/types/apiResponses';
...
export class PodcastService implements IPodcastService {
	...
	public async fetchTopPodcastsWithCache(): Promise<Podcast[]> {
		...
		const cachedData = this.cacheManager.getCache<Podcast[]>(cacheKey, cacheDuration);
	...
	public async fetchPodcastDetails(podcastId: string): Promise<PodcastDetailsAPIResponse> {
		...
		const rawData = await this.fetchFromApi<PodcastDetailsRawAPIResponse>(endpoint);
	...
	public async fetchPodcastDetailsWithCache(podcastId: string): Promise<PodcastDetailsAPIResponse> {
		const cacheKey = `podcast_${podcastId}`;
		const cacheDuration = 24 * 60 * 60 * 1000; // 24 horas
		const cachedData = this.cacheManager.getCache<PodcastDetailsAPIResponse>(cacheKey, cacheDuration);
